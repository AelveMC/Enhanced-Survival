...
# Function heals player if target is undead. Also sets target on fire.
...
execute as @e[type=#enhanced_weapons:undead,limit=1,distance=..8,nbt={HurtTime:10s}] at @s run execute as @e[type=#enhanced_weapons:undead,distance=..8] at @s run data merge entity @s {Fire:100}
